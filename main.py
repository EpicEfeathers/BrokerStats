...
async def on_message(message: discord.Message):
    ...
    if message.content.startswith(f"<@{bot.user.id}>") or message.content.startswith(f"<@!{bot.user.id}>"):
        # for funsies
        if message.author.id == 643617634419474432:
            await message.channel.send(f"Oh {message.author.display_name}, my glorious king {message.author.display_name}, you are such an amazing player :heart: :goat:")
            return
