...
async def squad_command(client, squad, interaction):
    ...
    if squad in client.squad_list:
        ...
        try:
            ...
            users = {user['name']: user['uid'] for user in stats['members']} # create this for the dropdown
            view = menu_paginator.first_view(interaction.user.id, users, squad, 180, client.session)
